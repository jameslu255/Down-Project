...
class SortView: UIViewController{
    ...
    override func viewDidLoad() {
        ...
        for n in 0...sortedCheck.count-1{
            ...
            {
                timeButton.setImage(UIImage(systemName: "stop.fill"), for: .normal)
            }
            if (n == 1 && sortedCheck[n] == 1){
                distanceButton.setImage(UIImage(systemName: "stop.fill"), for: .normal)
            }
            if (n == 2 && sortedCheck[n] == 1){
                downsButton.setImage(UIImage(systemName: "stop.fill"), for: .normal)
            }
        }
    }
    
    func resetAllButtons(){
        timeButton.setImage(UIImage(systemName: "stop"), for: .normal)
        distanceButton.setImage(UIImage(systemName: "stop"), for: .normal)
        downsButton.setImage(UIImage(systemName: "stop"), for: .normal)
        for n in 0...sortedCheck.count - 1{
            sortedCheck[n] = 0
        }
    }
